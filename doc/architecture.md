...
| `FWIR-CUTOVER-004` — no AST backend or runtime overload selection | verified-only backend signatures | `python:tools/validation/contracts.py::validate_product_cutover`<br>`rust:src/c_emitter.rs::every_selected_id_emits_direct_dispatch_without_type_redispatch` |
