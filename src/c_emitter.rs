...
impl<'a> IrCGenerator<'a> {
    ...
    fn emit_selected_kernel(
        ...
        let body = match kernel {
            ...
            ScalarKernel::LogDouble => {
                "fw_set_double(out,fw_backend_native_log(args[0].d));return 1;"
            }
            ...
            ScalarKernel::CosDouble => {
                "fw_set_double(out,fw_backend_native_cos(args[0].d));return 1;"
            }
            ...
            ScalarKernel::CeilDouble => {
                "fw_set_double(out,fw_backend_native_ceil(args[0].d));return 1;"
            }
            ScalarKernel::TruncDouble => {
                "fw_set_double(out,fw_backend_native_trunc(args[0].d));return 1;"
            }
            ScalarKernel::LengthBoolVector
            | ScalarKernel::LengthIntVector
            | ScalarKernel::LengthDoubleVector
...
static double fw_backend_native_log(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=log(input);
  fw_restore_strict_environment(&environment);
...
static double fw_backend_native_log10(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=log10(input);
  fw_restore_strict_environment(&environment);
...
static double fw_backend_native_cos(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=cos(input);
  fw_restore_strict_environment(&environment);
...
static double fw_backend_native_tan(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=tan(input);
  fw_restore_strict_environment(&environment);
...
static double fw_backend_native_ceil(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=ceil(input);
  fw_restore_strict_environment(&environment);
...
static double fw_backend_native_trunc(double value) {
  FWStrictEnvironment environment;volatile double input=value,result=0.0;
  fw_begin_strict_environment(&environment);
  result=trunc(input);
  fw_restore_strict_environment(&environment);
...
static int fw_main(int argc,char **argv,size_t root_count,const FWExpr *roots) {
  ...
  (void)fw_backend_native_ceil;
  (void)fw_backend_native_trunc;
...
mod ir_tests {
    ...
    fn every_selected_id_emits_direct_dispatch_without_type_redispatch() {
        ...
        assert!(source.contains("fw_apply_selected(fw_kernel_62,"));
        assert!(source.contains("static int fw_kernel_63("));
        assert!(source.contains("fw_apply_selected(fw_kernel_63,"));
