...
pub(crate) fn backend_native_log10(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::log10` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::log10(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
pub(crate) fn backend_native_sin(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::sin` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::sin(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
pub(crate) fn backend_native_cos(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::cos` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::cos(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
pub(crate) fn backend_native_tan(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::tan` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::tan(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
pub(crate) fn backend_native_floor(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::floor` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::floor(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
pub(crate) fn backend_native_trunc(value: f64) -> f64 {
    // SAFETY: `StrictEnvironment` saves the complete supported host state,
    // installs masked round-to-nearest with gradual underflow, and restores the
    // saved bytes in `Drop`. The volatile input/result keep the direct
    // `f64::trunc` call inside that guard.
    let result = unsafe {
        let environment = StrictEnvironment::begin();
        let strict_value = core::ptr::read_volatile(&value);
        let mut strict_result = f64::trunc(strict_value);
        let result = core::ptr::read_volatile(&strict_result);
        core::ptr::write_volatile(&mut strict_result, 0.0);
        drop(environment);
        result
...
mod tests {
    ...
    #[test]
    fn backend_native_log10_preserves_exact_special_values_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0xfff0_0000_0000_0000),
            (0x8000_0000_0000_0000, 0xfff0_0000_0000_0000),
            (0x3ff0_0000_0000_0000, 0x0000_0000_0000_0000),
            ...
            assert_eq!(
                backend_native_log10(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    #[test]
    fn backend_native_sin_preserves_signed_zero_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
            (0x8000_0000_0000_0000, 0x8000_0000_0000_0000),
            (0x7ff0_0000_0000_0000, CANONICAL_NAN_BITS),
            (0xfff0_0000_0000_0000, CANONICAL_NAN_BITS),
            (0x7ff8_0000_0000_0000, CANONICAL_NAN_BITS),
        ] {
            assert_eq!(
                backend_native_sin(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    #[test]
    fn backend_native_cos_preserves_zero_result_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0x3ff0_0000_0000_0000),
            (0x8000_0000_0000_0000, 0x3ff0_0000_0000_0000),
            (0x7ff0_0000_0000_0000, CANONICAL_NAN_BITS),
            ...
            assert_eq!(
                backend_native_cos(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    #[test]
    fn backend_native_tan_preserves_signed_zero_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
            (0x8000_0000_0000_0000, 0x8000_0000_0000_0000),
            (0x7ff0_0000_0000_0000, CANONICAL_NAN_BITS),
            ...
            assert_eq!(
                backend_native_tan(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    #[test]
    fn backend_native_floor_preserves_zeros_infinities_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
            (0x8000_0000_0000_0000, 0x8000_0000_0000_0000),
            (0x7ff0_0000_0000_0000, 0x7ff0_0000_0000_0000),
            ...
            assert_eq!(
                backend_native_floor(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    #[test]
    fn backend_native_trunc_preserves_zeros_infinities_and_canonical_nan() {
        for (input, expected) in [
            (0x0000_0000_0000_0000, 0x0000_0000_0000_0000),
            (0x8000_0000_0000_0000, 0x8000_0000_0000_0000),
            (0x7ff0_0000_0000_0000, 0x7ff0_0000_0000_0000),
            ...
            assert_eq!(
                backend_native_trunc(f64::from_bits(input)).to_bits(),
                expected
            );
        }
    }
    ...
    fn hostile_x86_environment_is_ignored_and_exactly_restored() {
        ...
        unsafe {
            ...
            let reference_log10 = backend_native_log10(f64::from_bits(1)).to_bits();
            let reference_sin = backend_native_sin(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let reference_cos = backend_native_cos(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            ...
            let reference_trunc =
                backend_native_trunc(f64::from_bits(0x8000_0000_0000_0001)).to_bits();
            ...
            let log10_result = backend_native_log10(f64::from_bits(1)).to_bits();
            let after_log10 = read_mxcsr();
            let sin_result = backend_native_sin(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_sin = read_mxcsr();
            let cos_result = backend_native_cos(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_cos = read_mxcsr();
            let tan_result = backend_native_tan(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_tan = read_mxcsr();
            let floor_result =
                backend_native_floor(f64::from_bits(0x8000_0000_0000_0001)).to_bits();
            ...
            assert_eq!(log10_result, reference_log10);
            assert_eq!(after_log10, hostile);
            assert_eq!(sin_result, reference_sin);
            assert_eq!(after_sin, hostile);
            assert_eq!(cos_result, reference_cos);
            assert_eq!(after_cos, hostile);
            assert_eq!(tan_result, reference_tan);
            ...
            assert_eq!(trunc_result, reference_trunc);
            assert_eq!(after_trunc, hostile);
    ...
    fn hostile_aarch64_environment_is_ignored_and_exactly_restored() {
        ...
        unsafe {
            ...
            let log10_result = backend_native_log10(f64::from_bits(1)).to_bits();
            let after_log10 = read_environment();
            let sin_result = backend_native_sin(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_sin = read_environment();
            let cos_result = backend_native_cos(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_cos = read_environment();
            let tan_result = backend_native_tan(f64::from_bits(0x7e37_e43c_8800_759c)).to_bits();
            let after_tan = read_environment();
            let floor_result =
                backend_native_floor(f64::from_bits(0x8000_0000_0000_0001)).to_bits();
            ...
            let trunc_result =
                backend_native_trunc(f64::from_bits(0x8000_0000_0000_0001)).to_bits();
            let after_trunc = read_environment();
