...
//! execution then uses [`evaluate_verified_program`]; the interpreter accepts
//! no parser AST and performs no overload selection.
//!
...
//! metadata, and interpreted programs execute with the caller's authority.
...
pub use fwir_api::{
    CompileFwirError, FwirInspectError, compile_source_to_fwir, compile_source_to_fwir_with_name,
    compile_source_to_verified_program, evaluate_verified_program_with_arguments, inspect_fwir,
};
