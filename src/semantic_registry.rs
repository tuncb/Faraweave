...
const fn scalar_kernel_name(value: ScalarKernel) -> &'static str {
    match value {
        ...
        ScalarKernel::CosDouble => "cos_double",
        ScalarKernel::TanDouble => "tan_double",
        ScalarKernel::FloorDouble => "floor_double",
        ScalarKernel::CeilDouble => "ceil_double",
        ScalarKernel::TruncDouble => "trunc_double",
    }
}
...
mod tests {
    ...
    fn production_registry_is_complete_and_numeric_lookups_are_checked() {
        ...
        assert_eq!(
            signature_from_numeric(59).map(|descriptor| descriptor.primitive_name),
            Ok("cos")
        );
        assert_eq!(
            implementation_from_numeric(59).map(|descriptor| descriptor.kernel),
            Ok(ScalarKernel::CosDouble)
        );
        assert_eq!(
            signature_from_numeric(60).map(|descriptor| descriptor.primitive_name),
            Ok("tan")
        );
        assert_eq!(
            implementation_from_numeric(60).map(|descriptor| descriptor.kernel),
            Ok(ScalarKernel::TanDouble)
        );
        assert_eq!(
            signature_from_numeric(61).map(|descriptor| descriptor.primitive_name),
            Ok("floor")
        );
        assert_eq!(
            implementation_from_numeric(61).map(|descriptor| descriptor.kernel),
            Ok(ScalarKernel::FloorDouble)
        );
        assert_eq!(
            signature_from_numeric(62).map(|descriptor| descriptor.primitive_name),
            Ok("ceil")
        );
        assert_eq!(
            implementation_from_numeric(62).map(|descriptor| descriptor.kernel),
            Ok(ScalarKernel::CeilDouble)
        );
        assert_eq!(
            signature_from_numeric(63).map(|descriptor| descriptor.primitive_name),
            Ok("trunc")
        );
        assert_eq!(
            implementation_from_numeric(63).map(|descriptor| descriptor.kernel),
