...
fn floor_node(bytes: &[u8]) -> usize {
    let (offset, length) = section(bytes, 14);
    bytes[offset..offset + length]
        .chunks_exact(56)
        .position(|record| {
            record[0] == 4
                && u32::from_le_bytes([record[24], record[25], record[26], record[27]]) == 36
        })
        .map(|index| offset + index * 56)
        .unwrap_or_else(|| panic!("canonical artifact lacks floor node"))
}
...
fn floor_resources_failures_cleanup_and_diagnostics_are_exact() {
    ...
    assert_eq!(allocation.primitive.as_deref(), Some("floor"));
    assert_eq!(
    ...
    let empty = evaluate_expression("floor[Double()]").expect("empty vector");
...
fn floor_fwir_roundtrip_malformed_identities_and_version_are_checked() {
    ...
    let decoded = decode_fwir(&bytes, &FwirDecodeLimits::default()).expect("decode floor artifact");
    assert_eq!(
        encode_fwir(&decoded, &FwirEncodeOptions::default()).expect("re-encode floor artifact"),
        bytes
    );
    assert_eq!(
    ...
    for (relative, replacement) in [(24, 35_u32), (28, 41), (32, 41)] {
        ...
        let error = decode_fwir(&malformed, &FwirDecodeLimits::default())
            .expect_err("mismatched floor identity");
        assert_eq!(
            error.kind,
    ...
    let error = decode_fwir(&old_version, &FwirDecodeLimits::default())
        .expect_err("semantic 1.0 floor artifact");
    assert!(matches!(
        error.kind,
        FwirDecodeErrorKind::MalformedProgram(VerifyError::MalformedProgram(ref malformed))
...
#[test]
fn floor_emitted_c_calls_math_h_floor_directly() {
    let source = emit_c_source("floor[(0.0 1.0)]\n")
        .expect("floor C emission")
        .source;
