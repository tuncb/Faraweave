...
fn trunc_node(bytes: &[u8]) -> usize {
    let (offset, length) = section(bytes, 14);
    bytes[offset..offset + length]
        .chunks_exact(56)
        .position(|record| {
            record[0] == 4
                && u32::from_le_bytes([record[24], record[25], record[26], record[27]]) == 38
        })
        .map(|index| offset + index * 56)
        .unwrap_or_else(|| panic!("canonical artifact lacks trunc node"))
}
...
fn trunc_resources_failures_cleanup_and_diagnostics_are_exact() {
    ...
    assert_eq!(allocation.primitive.as_deref(), Some("trunc"));
    assert_eq!(
    ...
    let empty = evaluate_expression("trunc[Double()]").expect("empty vector");
...
fn trunc_fwir_roundtrip_malformed_identities_and_version_are_checked() {
    ...
    let decoded = decode_fwir(&bytes, &FwirDecodeLimits::default()).expect("decode trunc artifact");
    assert_eq!(
        encode_fwir(&decoded, &FwirEncodeOptions::default()).expect("re-encode trunc artifact"),
        bytes
    );
    assert_eq!(
    ...
    let node = trunc_node(&bytes);
    for (relative, replacement) in [(24, 37_u32), (28, 43), (32, 43)] {
        ...
        let error = decode_fwir(&malformed, &FwirDecodeLimits::default())
            .expect_err("mismatched trunc identity");
        assert_eq!(
            error.kind,
    ...
    let error = decode_fwir(&old_version, &FwirDecodeLimits::default())
        .expect_err("semantic 1.0 trunc artifact");
    assert!(matches!(
        error.kind,
        FwirDecodeErrorKind::MalformedProgram(VerifyError::MalformedProgram(ref malformed))
...
#[test]
fn trunc_emitted_c_calls_math_h_trunc_directly() {
    let source = emit_c_source("trunc[(0.0 1.0)]\n")
        .expect("trunc C emission")
        .source;
    assert!(source.contains("#include <math.h>"));
    assert!(source.contains("result=trunc(input);"));
    assert!(source.contains("fw_set_double(out,fw_backend_native_trunc(args[0].d))"));
    assert!(source.contains("static int fw_kernel_44("));
    assert!(source.contains("static int fw_impl_44("));
}
